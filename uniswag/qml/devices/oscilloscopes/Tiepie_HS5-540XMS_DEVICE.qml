...
GridLayout {
    ...
    Label {
        id: triggerDelayLabel
        text: "Trigger Delay"
    }

    Label {
        id: triggerHoldOffLabel
        text: "Trigger Hold Off"
    }
    ...
    TextField {
        id: segmentCount
        onAccepted: {
            OscProperties._seg_cnt(text)
            updateDisplayedOscData()
        }

        background: Rectangle {
            implicitWidth: segmentCountLabel.width
            implicitHeight: 25
            radius: 2
    ...
    TextField {
        id: triggerDelay
        onAccepted: {
            OscProperties._trig_delay(text)
            updateDisplayedOscData()
        }

        background: Rectangle {
            implicitWidth: triggerDelayLabel.width
            implicitHeight: 25
            radius: 2
            color: background_color
            border.color: colorPalette.light
    ...
    TextField {
        id: triggerHoldOff
        onAccepted: {
            OscProperties._trig_holdoff(text)
            updateDisplayedOscData()
        }

        background: Rectangle {
            implicitWidth: triggerHoldOffLabel.width
            implicitHeight: 25
            radius: 2
            color: background_color
            border.color: colorPalette.light
    ...
    Connections {
        ...
        function onSegCnt(device_id, value) {
            if(compareDevices(device_id, oscilloscopeMainRect.selectedDevice)){
                segmentCount.text = ""
                segmentCount.placeholderText = value
            }
        }
        ...
        function onTrigDelay(device_id, value) {
            if(compareDevices(device_id, oscilloscopeMainRect.selectedDevice)){
                triggerDelay.text = ""
                triggerDelay.placeholderText = value
            }
        }
        ...
        function onTrigHoldoff(device_id, value) {
            if(compareDevices(device_id, oscilloscopeMainRect.selectedDevice)){
                triggerHoldOff.text = ""
                triggerHoldOff.placeholderText = value
            }
        }
